...
# # 👇 MODEL SETUP
# def get_chat_model():
#     # Hum 'gemini-flash-latest' use kar rahe hain
...
#     instruction = f"""
#     You are an HR Interviewer. Topic: {topic}, Level: {level}. Mode: {mode}.
#     Total Questions to ask: {total_questions}.
    
#     RULES:
#     1. Ask exactly ONE question at a time.
#     2. Wait for the user's answer.
#     3. Do NOT provide feedback immediately, just move to the next question.
#     """
    
#     if resume_text: instruction += f"\nUse this Resume context: {resume_text}"
...
#     try:
#         response = chat_session.send_message("Interview finished. Analyze all answers. Give Score (0-100) and Feedback.")
...
#             "feedback": full_text, 
#             "date": datetime.now().strftime("%d %b %Y, %I:%M %p"), # Format: 30 Jan 2026, 10:30 AM
#             "timestamp": datetime.now() # Sorting ke liye
#         })
#         return {"message": full_text}
#     except Exception as e:
#         return {"message": f"Error: {str(e)}"}

# # 👇 NEW ENDPOINT: DASHBOARD STATS (Real Data ke liye)
...
#         # 3. Recent History (Last 5 Fetch karo)
#         # '_id' ko exclude kiya kyunki ObjectId JSON serializable nahi hota direct
